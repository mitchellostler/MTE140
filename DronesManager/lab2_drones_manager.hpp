...
public:
	...
	DroneRecord create_member(unsigned int number) {
		return DroneRecord(number);
	}
	// ACCESSORS
	...
	// if not found, returns the size of the list; if the list is empty, returns -1
	int search(DroneRecord val) const;
	...
	// DO NOT store val's location into the list; instead copy it to dynamic memory
